...
;(function(root, factory) {
  ...
  function setupDOM() {
    ...
    // Arrow controls
    dom.controls = createSingletonNode(
      dom.wrapper,
      'aside',
      'controls',
    ...
    dom.pauseOverlay = createSingletonNode(
      dom.wrapper,
      'div',
      'pause-overlay',
    ...
    if (config.help) {
      ...
      for (var binding in registeredKeyBindings) {
        if (registeredKeyBindings[binding].key && registeredKeyBindings[binding].description) {
          html +=
            '<tr><td>' +
            registeredKeyBindings[binding].key +
            '</td><td>' +
  ...
  function updateParallax() {
    if (config.parallaxBackgroundImage) {
      ...
      if (typeof config.parallaxBackgroundHorizontal === 'number') {
        ...
        horizontalOffsetMultiplier =
          horizontalSlideCount > 1 ? (backgroundWidth - slideWidth) / (horizontalSlideCount - 1) : 0
      }
  ...
  function formatEmbeddedContent() {
    var _appendParamToIframeSource = function(sourceAttribute, sourceURL, param) {
      toArray(dom.slides.querySelectorAll('iframe[' + sourceAttribute + '*="' + sourceURL + '"]')).forEach(function(
        el
      ) {
