  ...
  function a(e) {
    ;(this.tokens = []),
      ...
      this.options.pedantic
        ...
        : this.options.gfm && (this.options.tables ? (this.rules = k.tables) : (this.rules = k.gfm))
  }
  ;(k._label = /(?!\s*\])(?:\\[\[\]]|[^\[\]])+/),
    (k._title = /(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/),
    (k.def = i(k.def)
    ...
    (k.html = i(k.html, 'i')
      ...
      .getRegex()),
    (k.paragraph = i(k.paragraph)
      .replace('hr', k.hr)
      .replace('heading', k.heading)
      .replace('lheading', k.lheading)
      .replace('tag', k._tag)
      .getRegex()),
    (k.blockquote = i(k.blockquote)
    ...
    (k.gfm.paragraph = i(k.paragraph)
      .replace(
        '(?!',
        '(?!' + k.gfm.fences.source.replace('\\1', '\\2') + '|' + k.list.source.replace('\\1', '\\3') + '|'
      )
      .getRegex()),
    (k.tables = d({}, k.gfm, {
      nptable: /^ *([^|\n ].*\|.*)\n *([-:]+ *\|[-| :]*)(?:\n((?:.*[^>\n ].*(?:\n|$))*)\n*|$)/,
      ...
      for (e = e.replace(/^ +$/gm, ''); e; )
        if (
          ((s = this.rules.newline.exec(e)) &&
            ((e = e.substring(s[0].length)), 1 < s[0].length && this.tokens.push({ type: 'space' })),
          (s = this.rules.code.exec(e)))
        )
          (e = e.substring(s[0].length)),
            (s = s[0].replace(/^ {4}/gm, '')),
            this.tokens.push({ type: 'code', text: this.options.pedantic ? s : y(s, '\n') })
        else if ((s = this.rules.fences.exec(e)))
          (e = e.substring(s[0].length)),
            this.tokens.push({ type: 'code', lang: s[2] ? s[2].trim() : s[2], text: s[3] || '' })
        else if ((s = this.rules.heading.exec(e)))
          (e = e.substring(s[0].length)), this.tokens.push({ type: 'heading', depth: s[1].length, text: s[2] })
        else if (
          t &&
          ...
          this.tokens.push(o)
        } else if ((s = this.rules.hr.exec(e))) (e = e.substring(s[0].length)), this.tokens.push({ type: 'hr' })
          ...
          for (
            e = e.substring(s[0].length),
              a = { type: 'list_start', ordered: (d = 1 < (i = s[2]).length), start: d ? +i : '', loose: !1 },
              this.tokens.push(a),
              n = !(h = []),
            ...
            (u = (o = s[c]).length),
              ...
              c !== f - 1 &&
                ...
                (1 < i.length ? 1 === l.length : 1 < l.length || (this.options.smartLists && l !== i)) &&
                  ((e = s.slice(c + 1).join('\n') + e), (c = f - 1))),
              (r = n || /\n\n(?!\s*$)/.test(o)),
              c !== f - 1 && ((n = '\n' === o.charAt(o.length - 1)), r || (r = n)),
              r && (a.loose = !0),
              (b = void 0),
              (m = /^\[[ xX]\] /.test(o)) && ((b = ' ' !== o[1]), (o = o.replace(/^\[[ xX]\] +/, ''))),
              (p = { type: 'list_item_start', task: m, checked: b, loose: r }),
              h.push(p),
              this.tokens.push(p),
              this.token(o, !1),
          ...
          (e = e.substring(s[0].length)),
            ...
            this.tokens.links[g] || (this.tokens.links[g] = { href: s[2], title: s[3] })
        else if (
          t &&
          ...
          for (c = 0; c < o.cells.length; c++)
            o.cells[c] = x(o.cells[c].replace(/^ *\| *| *\| *$/g, ''), o.header.length)
          this.tokens.push(o)
        } else if ((s = this.rules.lheading.exec(e)))
          (e = e.substring(s[0].length)), this.tokens.push({ type: 'heading', depth: '=' === s[2] ? 1 : 2, text: s[1] })
          ...
          (e = e.substring(s[0].length)), this.tokens.push({ type: 'text', text: s[0] })
  ...
  ;(n._punctuation = '!"#$%&\'()*+,\\-./:;<=>?@\\[^_{|}~'),
        ...
        if ((i = this.rules.escape.exec(e))) (e = e.substring(i[0].length)), (o += u(i[1]))
          ...
          !this.inLink && /^<a /i.test(i[0])
            ...
            !this.inRawBlock && /^<(pre|code|kbd|script)(\s|>)/i.test(i[0])
              ? (this.inRawBlock = !0)
              : this.inRawBlock && /^<\/(pre|code|kbd|script)(\s|>)/i.test(i[0]) && (this.inRawBlock = !1),
            (e = e.substring(i[0].length)),
            (o += this.options.sanitize ? (this.options.sanitizer ? this.options.sanitizer(i[0]) : u(i[0])) : i[0])
          ...
          (e = e.substring(i[0].length)),
            ...
            (o += this.outputLink(i, { href: h.escapes(r), title: h.escapes(s) })),
            (this.inLink = !1)
          ...
          (e = e.substring(i[0].length)), (o += this.renderer.strong(this.output(i[4] || i[3] || i[2] || i[1])))
        else if ((i = this.rules.em.exec(e)))
          (e = e.substring(i[0].length)),
            (o += this.renderer.em(this.output(i[6] || i[5] || i[4] || i[3] || i[2] || i[1])))
        else if ((i = this.rules.code.exec(e)))
          (e = e.substring(i[0].length)), (o += this.renderer.codespan(u(i[2].trim(), !0)))
          ...
          if ((i = this.rules.text.exec(e)))
            (e = e.substring(i[0].length)),
              this.inRawBlock ? (o += this.renderer.text(i[0])) : (o += this.renderer.text(u(this.smartypants(i[0]))))
          else if (e) throw new Error('Infinite loop on byte: ' + e.charCodeAt(0))
        } else {
    ...
    (h.prototype.mangle = function(e) {
      ...
      for (var t, n = '', r = e.length, s = 0; s < r; s++)
        (t = e.charCodeAt(s)), 0.5 < Math.random() && (t = 'x' + t.toString(16)), (n += '&#' + t + ';')
      return n
    }),
    (r.prototype.code = function(e, t, n) {
      ...
      return r
        ? '<pre><code class="' + this.options.langPrefix + u(r, !0) + '">' + (n ? e : u(e, !0)) + '</code></pre>\n'
    ...
    (r.prototype.checkbox = function(e) {
      return (
        '<input ' + (e ? 'checked="" ' : '') + 'disabled="" type="checkbox"' + (this.options.xhtml ? ' /' : '') + '> '
      )
    }),
    ...
    (p.prototype.parse = function(e) {
      ;(this.inline = new h(e.links, this.options)),
        (this.inlineText = new h(e.links, d({}, this.options, { renderer: new s() }))),
  ...
  function m(e, n, r) {
    ...
    if (r || 'function' == typeof n) {
      ...
      try {
        ...
        if (((e.message += '\nPlease report this to https://github.com/markedjs/marked.'), (n || m.defaults).silent))
          return '<p>An error occurred:</p><pre>' + u(e.message + '', !0) + '</pre>'
        throw e
      }
  }
  ;(f.exec = f),
